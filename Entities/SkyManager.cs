    ...
    public class SkyManager : IGameEntity, IDayNightCycle
    {
        ...
        public void Draw(SpriteBatch spriteBatch, GameTime gameTime)
        {
            if(OverlayVisibility > EPSILON)
                spriteBatch.Draw(_overlay, new Rectangle(0, 0, DinoRunnerGame.WINDOW_WIDTH, DinoRunnerGame.WINDOW_HEIGHT), Color.White * OverlayVisibility);
        ...
        public void Update(GameTime gameTime)
        {
            if(_moon == null)
            {
                _moon = new Moon(this, _spriteSheet, _trex, new Vector2(DinoRunnerGame.WINDOW_WIDTH, MOON_POS_Y));
            ...
            foreach(SkyObject skyObject in _entityManager.GetEntitiesOfType<SkyObject>().Where(s => s.Position.X < -100))
            {
                ...
                {
                    moon.Position = new Vector2(DinoRunnerGame.WINDOW_WIDTH, MOON_POS_Y);
                }
                else
        ...
        private void HandleCloudSpawning()
        {
            ...
            if (clouds.Count() <= 0 || (DinoRunnerGame.WINDOW_WIDTH - clouds.Max(c => c.Position.X)) >= _targetCloudDistance)
            {
                ...
                Cloud cloud = new Cloud(_spriteSheet, _trex, new Vector2(DinoRunnerGame.WINDOW_WIDTH, posY));
        ...
        private void HandleStarSpawning()
        {
            ...
            if(stars.Count() <= 0 || (DinoRunnerGame.WINDOW_WIDTH - stars.Max(c => c.Position.X)) >= _targetStarDistance)
            {
                ...
                Star star = new Star(this, _spriteSheet, _trex, new Vector2(DinoRunnerGame.WINDOW_WIDTH, posY));
